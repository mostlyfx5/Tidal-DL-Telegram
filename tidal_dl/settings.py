...
class TokenSettings(ModelBase):
    ...
    def save(model):
        ...
        txt = __encode__(txt)
        path = TokenSettings.__getFilePath__()
        set_db.set_variable("AUTH_TOKEN", 0, True, txt)
        write(path, txt, 'wb')
...
class Settings(ModelBase):
    ...
    albumFolderFormat = R"{ArtistsName}/{Flag} {AlbumTitle} [{AlbumID}] [{AlbumYear}]"
    ...
    def getDefaultAlbumFolderFormat():
        return R"{ArtistsName}/{Flag} {AlbumTitle} [{AlbumID}] [{AlbumYear}]"
